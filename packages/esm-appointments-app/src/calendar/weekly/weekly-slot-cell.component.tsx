...
interface WeeklySlotCellProps {
  /** Appointments for this day (pre-filtered to the time block or unfiltered). */
...
/**
 * Renders a single cell in the weekly calendar grid.
 *
 * Shows up to 2 compact appointment previews (time + patient name).
 * If more than 2, renders a "+N more" Carbon Link that opens the
 * existing day-appointments modal via onSelectDate.
 */
const WeeklySlotCell: React.FC<WeeklySlotCellProps> = ({
  ...
  // ── Filter & sort appointments for this time block ──────────────────────
  ...
  if (!hasAppts) {
    return <div className={`${styles.slotCell} ${isToday ? styles.slotCellToday : ''}`} />;
  }
